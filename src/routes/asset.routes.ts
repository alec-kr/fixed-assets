...
import { withTenant, PgCtx } from '../db';
import {
  createAsset,
  createAssetSchema,
  listAssets,
...
// Pull ctx from middleware (server.ts) and allow an optional "reason"
function buildCtx(req: any): PgCtx {
  const base = (req as any)._pgCtx || {};
  const reason =
...
router.post('/', async (req, res, next) => {
  try {
    ...
    const ctx = buildCtx(req); // optional: set x-reason: create

    const asset = await withTenant(
      tenantId,
      async (c) => createAsset(c, parsed.data),
      ctx
    );
    ...
    if (e.code === '23505') {
      return res.status(409).json({ error: 'asset_tag exists for tenant' });
    }
    next(e);
  }
});
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    const row = await withTenant(tenantId, async (c) => {
      const { rows } = await c.query(
        'select * from assets where id = $1::uuid',
        [req.params.id]
      );
      return rows[0];
    });
...
router.patch('/:id', async (req, res, next) => {
  try {
    ...
    const ctx = buildCtx(req); // e.g. reason=revaluation/relocation/etc.

    const updated = await withTenant(
      tenantId,
      async (c) => updateAsset(c, parsed.data),
      ctx
    );
...
router.patch('/by-tag/:tag', async (req, res, next) => {
  try {
    ...
    const cost =
      typeof req.body.acquisition_cost === 'string' &&
      req.body.acquisition_cost.trim() !== ''
        ? req.body.acquisition_cost.trim()
        : undefined;

    const loc =
      typeof req.body.location_id === 'string' &&
      req.body.location_id.trim() !== ''
        ? req.body.location_id.trim()
        : undefined;
    ...
    if (cost) {
      sets.push(`acquisition_cost = $${i++}`);
      params.push(cost);
    }
    if (loc) {
      sets.push(`location_id = $${i++}::uuid`);
      params.push(loc);
    }
